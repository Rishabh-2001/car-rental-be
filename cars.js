...
//     "nextAvaiableDate":  "",
//     "ratings":"4",
//     "SecurityDep": "30000 INR",
//     "PerDayCharge": "2000",
//     "lateChargePerDay":2500,
//     "discount": "NA",

// }

// have to add agency link
function generateRandomCarData(cars, carsImages) {
  const mileages = [
    "25kmpl",
    "26kmpl",
    "27kmpl",
  ...
  const seatingCaps = [3, 4, 5, 6, 7, 8, 9, 10];
  ...
  const generateRandomDate = () => {
    ...
    const randomDays = Math.floor(Math.random() * 30); // Random number of days
    today.setDate(today.getDate() + randomDays);
    return today.toISOString().split("T")[0];
  };

  const generateRandomInsuranceExp = () => {
    const today = new Date();
    const randomMonths = Math.floor(Math.random() * 12); // Random number of months
    today.setMonth(today.getMonth() + randomMonths);
    return today.toISOString().split("T")[0];
  };
  ...
  const generateRandomCar = (car) => {
    return {
      ...car,
      Mileage: getRandomElement(mileages),
      FuelType: getRandomElement(fuelTypes),
      SeatingCapacity: getRandomElement(seatingCaps),
      registration: Math.random().toString(36).substring(2, 10), // Generate a random registration number
      gpsEnabled: Math.random() < 0.5 ? "Yes" : "No",
      Insurance: Math.random() < 0.5 ? "Yes" : "No",
      InsuranceExp: Math.random() < 0.5 ? generateRandomInsuranceExp() : "",
      Location: "India",
      images: {
        url: carImages[Math.floor(Math.random() * carImages.length)].image,
      },
      nextAvaiableDate: generateRandomDate(),
      discount: "",
      ratings: "4",
      SecurityDep: "30000 INR",
      PerDayCharge: "2000",
  ...
  return cars.map((car) => generateRandomCar(car));
}
const carsWithRandomData = generateRandomCarData(cars, carImages);

// console.log(carsWithRandomData);
// saveCarsToDatabase(carsWithRandomData)

// Function to map keys to columns
function mapKeysToColumns(carObj) {
  return {
    //   car: carObj.car,
    carModel: carObj.car_model,
    carColor: carObj.car_color,
    carModelYear: carObj.car_model_year,
    carVin: carObj.car_vin,
    carPrice: carObj.price,
    availability: carObj.availability,
    mileage: carObj.Mileage,
    fuelType: carObj.FuelType,
    seatingCapacity: carObj.SeatingCapacity,
    registration: carObj.registration,
    gpsEnabled: carObj.gpsEnabled,
    Insurance: carObj.Insurance,
    InsuranceExp: carObj.InsuranceExp,
    Location: carObj.Location,
    images: carObj.images,
    nextAvailableDate: carObj.nextAvaiableDate,
    discount: carObj.discount,
    ratings: carObj.ratings,
    SecurityDep: carObj.SecurityDep,
    PerDayCharge: carObj.PerDayCharge,
    lateChargePerDay: carObj.lateChargePerDay,
    features: carObj.features,
    rentalHistory: carObj.rentalHistory,
  };
}

async function saveCarsToDatabase(carsArray) {
  //    const d= {
  //     id: 99,
  //     car: 'Ford',
  //     car_model: 'F150',
  ...
  //     FuelType: 'Diesel',
  //     SeatingCapacity: 3,
  //     registration: 'ec7mk77z',
  //     gpsEnabled: 'Yes',
  //     Insurance: 'Yes',
  //     InsuranceExp: '2024-04-23',
  //     Location: 'India',
  //     images: {
  ...
  for (const carObj of carsArray) {
    //   const mappedCarObj = mapKeysToColumns(carObj);
    await Cars.create({
      car: carObj.car,
      carModel: carObj.car_model,
      carColor: carObj.car_color,
      ...
      features: carObj.features,
      rentalHistory: carObj.rentalHistory,
      vendorId: "9ba7cbe9-a7af-42da-9194-ed3908d30a18",
    });
  }
